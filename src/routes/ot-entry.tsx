            ...
            <table className="border-collapse text-xs w-full min-w-[1600px]">
              ...
              <thead className="bg-slate-100">
                <tr className="h-9">
                  ...
                  <th rowSpan={2} className="border-r border-b border-t bg-slate-100 px-2 py-2 min-w-[90px] sticky top-0 z-20 bg-clip-padding font-semibold">Time (OT Hrs)</th>
              ...
              {contractors.length > 0 && displayCells.length > 0 && (
                <tfoot>
                  <tr className="bg-yellow-100 font-bold">
                    ...
                    {displayGroups.flatMap((g) => g.cells.map((col, ci) => {
                      const isLastInGroup = ci === g.cells.length - 1;
                      const borderCls = isLastInGroup ? "border-r-2 border-r-slate-300" : "border-r border-r-slate-200";
                      return (<td key={col.key} className={cn("border-b border-t text-center", borderCls)}>{colTotals[col.key] || ""}</td>);
                    }))}
