...
function ContractorsPage() {
  ...
  const [form, setForm] = useState({ company_name: "", contact_person: "", phone: "", license_number: "" });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setOpen(false); setEditing(null); setForm({ company_name: "", contact_person: "", phone: "", license_number: "" }); load();
    } catch (err: any) { toast.error(err.message); }
  };

  const handleEdit = (c: any) => { setEditing(c); setForm({ company_name: c.company_name, contact_person: c.contact_person || "", phone: c.phone || "", license_number: c.license_number || "" }); setOpen(true); };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) { setEditing(null); setForm({ company_name: "", contact_person: "", phone: "", license_number: "" }); } }}>
      ...
      <Card><CardContent className="p-0">
        <Table>
          <TableHeader><TableRow><TableHead>Company Name</TableHead><TableHead>Contact Person</TableHead><TableHead>Phone</TableHead><TableHead>License #</TableHead><TableHead className="w-24">Actions</TableHead></TableRow></TableHeader>
          <TableBody>
            {filtered.map((c) => (
              <TableRow key={c.id}>
            ...
            {filtered.length === 0 && <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground py-8">No contractors found</TableCell></TableRow>}
          </TableBody>
        </Table>
      </CardContent></Card>
    </div>
