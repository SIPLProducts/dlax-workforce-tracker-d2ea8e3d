...
function CategoriesPage() {
  ...
  const load = async () => {
    const { data } = await supabase.from("worker_categories").select("*").order("name");
    setItems(data || []);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        const { error } = await supabase.from("worker_categories").update({ name }).eq("id", editing.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("worker_categories").insert({ name });
        if (error) throw error;
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Worker Categories</h1><p className="text-sm text-muted-foreground">Manage worker categories</p></div>
        <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) { setEditing(null); setName(""); } }}>
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <div><Label>Name *</Label><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Skilled, Unskilled, Helper" /></div>
      ...
      <Card><CardContent className="p-0">
        <Table>
          ...
          <TableBody>
            ...
            {filtered.length === 0 && <TableRow><TableCell colSpan={2} className="text-center text-muted-foreground py-8">No categories found</TableCell></TableRow>}
          </TableBody>
        </Table>
      </CardContent></Card>
    </div>
